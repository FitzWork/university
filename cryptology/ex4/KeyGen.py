def generate_key(k):
    ...
    for i in range(45):
        if i < 4:
            ...
            w[i] = w[i - 4] ^ rcon(i // 4) ^ sub_word(w[i - 1])
        else:
            w[i] = w[i - 4] ^ w[i - 1]
...
def sub_word(w):
    sBox = [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76, 0xca, 0x82,
            ...
            0xb0, 0x54, 0xbb, 0x16]
    w = bytes(w)
    return sBox[w[1]] + sBox[w[2]] + sBox[w[3]] + sBox[w[0]]


def aes_any_length(message: bin):
    if message != 0 % 128:
        message <<= message % 128

    message_parts = [] * message // 128

    for i in range(message // 128):
        message_parts[i] = message[(i * 128):(i * 128 + 128)]

    result = bin
    for i in range(len(message_parts)):
        result += aes(message_parts[i])
