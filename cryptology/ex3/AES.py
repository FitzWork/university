...
sBox = [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76, 0xca, 0x82,
        ...
        0xb0, 0x54, 0xbb, 0x16]
sBoxInverse = [0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb,
...
block = [0x5c, 0xf6, 0xee, 0x79,
         0x2c, 0xdf, 0x05, 0xe1,
         0xba, 0x2b, 0x63, 0x25,
         0xc4, 0x1a, 0x5f, 0x10]
roundKey = [[0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c],
...
def addRoundKey(key):
    global block, roundKey
    print(type(block[0]), block[0])
    for i in range(16):
        block[i] = hex(int(block[i]) ^ int(roundKey[key][i]))
...
def shiftRows():
    global block
    tmp = [block[0], block[5], block[10], block[15],
...
def mixColumns():
    ...
    for i in range(4):
        tmp[i][0] = xTime(block[i], 2) ^ xTime(block[i + 4], 3) ^ xTime(block[i + 8], 1) ^ xTime(block[i + 12], 1)
        tmp[i][1] = xTime(block[i], 1) ^ xTime(block[i + 4], 2) ^ xTime(block[i + 8], 3) ^ xTime(block[i + 12], 1)
        tmp[i][2] = xTime(block[i], 1) ^ xTime(block[i + 4], 1) ^ xTime(block[i + 8], 2) ^ xTime(block[i + 12], 3)
        tmp[i][3] = xTime(block[i], 3) ^ xTime(block[i + 4], 1) ^ xTime(block[i + 8], 1) ^ xTime(block[i + 12], 2)

    block = tmp


def xTime(text, times):
    if times == 1:
        return text
    ...
    if times == 2:
        return tmp if text < 128 else tmp ^ 0x1b
    if times == 3:
        return xTime(text, 2) ^ text
